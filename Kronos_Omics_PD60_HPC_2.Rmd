...
```{r}
#n = c(rows, columns) això és només per veure si les dates s'han importat bé
...
fit_df_hpc_sinusoid <- fit_df %>%  filter(Ethanol_p.val < 0.05 & Water_p.val <0.05)

# Generate the plot
plot <- gg_kronos_acrogram(fit_df_hpc_sinusoid) +
