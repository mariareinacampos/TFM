...
```{r}
library(readxl)
bigdata_PD35_HPC <- read_excel("/Users/mariareinacampos/Documents/00_MASTER/TFM/Resultats/Results_PD35_HPC/bigdata_PD35_HPC.xlsx")
...
```{r}
#n = c(rows, columns)
print(bigdata_PD35_HPC, n = c(51, 55)) #52 files comptant la cabecera, 51 sense la fila inicial
dim(bigdata_PD35_HPC)
head(bigmeta_PD35_HPC)
dim(bigmeta_PD35_HPC)
...
file_path <- "/Users/mariareinacampos/Documents/00_MASTER/TFM/Resultats/Results_PD35_HPC/RhythmicityResults_PD35_HPC.csv"
...
file_name <- paste0(output_folder, "/pvalue_PD35_HPC.png")
...
# Define the file name
file_name <- paste0(output_folder, "/acrogram_PD35_HPC.png")
...
```{r}
pairwise_list = vector(mode = "list", length = length(out_list_PD35_HPC)) 
for(m in 1:length(out_list_PD35_HPC)){
  pairwise_list[[m]] <- out_list_PD35_HPC [[m]]@pairwise_models
  names(pairwise_list)[m] <- names(out_list_PD35_HPC)[m]}
...
plot_list <- vector(mode = "list", length = length(out_list_PD35_HPC))
for(q in 1:length(out_list_PD35_HPC)){
  
  plot_list[[q]] <- gg_kronos_sinusoid_noNA(out_list_PD35_HPC[[q]]) +
  ...
  pdf("Plots_circadian_PD35_HPC_2.pdf")
...
for(q in 1:length(out_list_PD35_HPC)){
  ...
  pdf("Plots_circadian_PD35_HPC_3.pdf")
for (i in 1:length(plot_list)) {
  print(plot_list[[i]])
}
invisible(dev.off())
...
cor(t(bigdata_PD35_HPC[,-1]))
corrplot(cor(t(bigdata_PD35_HPC[,-1])))
#Pendent: corrgeir els missing values per poder fer les correlacions entre els gens. Crec que esta fet

#Heatmap
heatmap(as.matrix(t(bigdata_PD35_HPC[,-1])),
        scale = "col", labCol = bigdata_PD35_HPC$Genes, col = c("red","blue"))
...
gost(altered_genes_names, organism = "mmusculus", significant = FALSE, correction_method = "fdr", custom_bg = bigdata_PD35_HPC$Genes)
